                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="paytab" role="tabpanel" aria-labelledby="pills-one-example2-tab">
                                    <div class="row justify-content-around">
                                        <div class="card">
                                            <div class="card-body">
                                               <h3>@lang('content.shipment_booking_id'): {{ $shipmentbook->id }}</h3>
                                               @if(app()->getLocale() == 'ur')
                                               <p><b>@lang('content.trip_title'):</b> {{ $shipmentbook->trip->title_urdu }}</p>
                                               @elseif(app()->getLocale() == 'ar')
                                                    <p><b>@lang('content.trip_title'):</b> {{ $shipmentbook->trip->title_arabic }}</p>
                                               @else
                                                    <p><b>@lang('content.trip_title'):</b> {{ $shipmentbook->trip->title }}</p>
                                               @endif
                                            </div>
                                        </div>
                                    </div>
